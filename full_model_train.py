...
from model.mental2 import MENTAL2
...
def run_train(learn_rate, wd, batch_sz, epochs, output_sz, outfile):
    ...
    main_dataset = SplitDataset('complete_samples.npy', '/Users/owenflynn/research/MENTAL2/TD-BRAIN-sample/imputed_features')
    ...
    model = MENTAL2(512, 120, 26, 5, 512, 1, 0.1)
    ...
    for epoch in range(epochs):
        ...
        for (dem, neo, psd, label) in train_loader:
            label_reshaped = np.reshape(label, (batch_sz, output_sz))

            output = model.forward(psd, neo, dem)
        ...
        cvals = []
        fvals = []
        for (dem, neo, psd, label) in test_loader:
...
run_train(learn_rate= learn, wd=weight_decay, batch_sz=2, epochs= 100, output_sz=1, outfile="results/test1")
