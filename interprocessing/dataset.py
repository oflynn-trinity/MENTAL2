...
class SplitDataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        dem = individual[1:4].astype(np.float32)

        neo = individual[4:64].astype(np.float32)

        psd = individual[64:]
        
        psd = psd.astype(float)
        psd_val = torch.tensor(psd, dtype=torch.float32)
        psd = torch.reshape(psd_val, [120,26,5])

        return dem, neo, psd, output
...
class ImputingDataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return individual[1:].astype(np.float32)
...
class ImputingMissingDataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        ind = self.individuals[idx][1:].astype(np.float32)
