...
def end2end_alphaPowerandiAPF(varargs):
    ...
    def computeFFT(data, Fs=500):
        ...
        freqs = np.linspace(0,Fs/2,int(len(power)/2)) #power1
    ...
    for f in preprocs.files:
        ...
        sid = f.rsplit('/')[-4][4:]; sessid = f.rsplit('/')[-3][-1:]
        ...
        print("subID = ",tsvdat['subID'])
        print("sid = ", sid)
        print("sessID = ",tsvdat['sessID'])
        print("sessid = ", sessid)

        try:

            idx = np.where((tsvdat['subID']==int(sid)) & (tsvdat['sessID']==int(sessid)))[0][0]
    ...
    print("output, ", output)
    age = np.squeeze(np.array(output['age'][1:]))
    gender = np.array(output['sex'][1:])
    power = np.array(output['power'][1:])
    conds = np.array(output['conds'][1:])
    idcodes = np.array(output['IDcodes'][1:])
    freqs= output['freqs']
    ...
    for s in np.unique(idcodes):
        idxs=[0]
        try:
            idxs = [(np.where((idcodes==s) & (conds=='task-restEC')))[0][0],(np.where((idcodes==s) & (conds=='task-restEO')))[0][0]]
        except:
            pass
        print("idxs, ", idxs)
        if len(idxs)==2:
    ...
    print("meanEC, meanEO = ",meanEC,meanEO)
    ...
    EC = np.where(conds=='task-restEC')[0]
    EO = np.where(conds=='task-restEO')[0]
    ECpower = power[EC,:]
    EOpower = power[EO,:]
    ...
    condidx = np.where(conds =='task-restEC')[0]
    age1 = age[condidx]
    powa = power[condidx]
